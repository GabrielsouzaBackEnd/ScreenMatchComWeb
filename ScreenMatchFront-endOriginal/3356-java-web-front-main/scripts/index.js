...
function criarListaFilmes(elemento, dados, maxItems = null) {
    const carousel = elemento.querySelector('.carousel');
    const ulExistente = carousel.querySelector('.lista');

    if (ulExistente) {
        carousel.removeChild(ulExistente);
    }

    const ul = document.createElement('ul');
    ul.className = 'lista';
    const filmes = maxItems ? dados.slice(0, maxItems) : dados;
    const listaHTML = filmes.map((filme) => `
        <li>
    ...
    configurarControles(carousel, ul);
}

function configurarControles(carousel, ul) {
...
function geraSeries() {
    ...
    Promise.all(urls.map(url => getDados(url)))
        .then(data => {
            criarListaFilmes(elementos.top5, data[0], 5);
            criarListaFilmes(elementos.lancamentos, data[1], 5);
            criarListaFilmes(elementos.series, data[2], 5);
        })
        .catch(error => {
